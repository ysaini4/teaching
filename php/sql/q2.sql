select temp2.*,apphistory.num_applied from ( 
 ...
 (company.deadline-38) as latetime,
 (company.deptt LIKE concat('%,',student.deptt,',%')) as ismydeptt
  from company left join users on users.id=cid  ,student where student.sid=38 AND company.conf='1' )
  temp left join rcmnd on ((rcmnd.sid=temp.sid  AND rcmnd.cid=temp.cid) ))temp2 
 left join 
(select cid,count(*) as num_applied from apply where sid=38 group by cid) apphistory
